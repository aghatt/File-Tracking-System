...
$query="select * from master_table_log where r_date BETWEEN '$from_date' and '$to_date' order by r_date";
if(($_POST['dpt']!='')&&($_POST['uid'])==''){
	$dpt=$_POST['dpt'];
	
	$query="select * from master_table_log where dst='$dpt' and r_date BETWEEN '$from_date' and '$to_date' order by r_date";
}
if(($_POST['dpt']=='')&&($_POST['uid']!='')){
	$uid=$_POST['uid'];
	
	$query="select * from master_table_log where uid='$uid' and r_date BETWEEN '$from_date' and '$to_date' order by r_date";
}
if(($_POST['uid']!='')&&($_POST['dpt']!='')){
	$uid=$_POST['uid'];
	$dpt=$_POST['dpt'];
	
	$query="select * from master_table_log where uid='$uid' and dst='$dpt' and r_date BETWEEN '$from_date' and '$to_date' order by r_date";
}
...
<?php 
	$j=0;
				for($j=0;$j<$i;$j++){
					...
					$src = $row['src'];	
					$dst = $row['dst'];
					$receive_date=$row['r_date'];
					$dt = explode('-',$row['r_date']);
